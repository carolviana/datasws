...
public class ArticleDAO {
	...
	public static void initializeArticlesStatus() {
		String statusString = 
	            "{\n" + 
	            "	  	\"status\": [\n" + 
        ...
        try (RepositoryConnection conn = repository.getConnection()) {
        	for (int i = 0; i < status.size(); i++) {
        		...
        		builder.subject(uriBase + "status/" + status.getJsonObject(i).getString("label"))
					   ...
					   .add("schema:creativeWorkStatus", factory.createIRI(uriBase + status.getJsonObject(i).getString("label")))
	...
	public static String createArticle(JsonObject article) {
		...
		builder.subject(uriBase + article.getInt("id"))
			   ...
			   .add("schema:creativeWorkStatus", factory.createIRI(article.getString("creativeWorkStatus")));
			...
			for(Iterator<Entry<String, JsonValue>> iterator = json.entrySet().iterator(); iterator.hasNext();) {
				...
				switch (key.toString()) {
					...
					case "creativeWorkStatus":{
						IRI object = factory.createIRI(json.getString(key));
